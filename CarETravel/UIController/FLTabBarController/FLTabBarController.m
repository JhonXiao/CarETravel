...
@interface FLTabBarController (){
    ...
    NSUInteger preIndex;//当点击联系客服是，保存上一个点击的index
    NSUInteger currentIndex;//当前点击的index
}
...
-(void)initUI{
    ...
    _barView = [[UIView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - BARHEIGHT - 64, [UIScreen mainScreen].bounds.size.width, BARHEIGHT)];
...
//        UIImageView *itemImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 30, 30)];
        UIImageView *itemImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 20, 20)];
...
-(void)setSelectedIndex:(NSUInteger)selectedIndex{
    ...
    if (selectedIndex == 0) {
        [self notificationHideLeftNaviBar:YES];
    }else if (selectedIndex == 1){
        [self notificationHideLeftNaviBar:YES];
    }else if (selectedIndex == 2){
        [self notificationHideLeftNaviBar:NO];
    }
