...
@interface ScanViewController ()<AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate,NetSocketDelegate>{
...
- (void)creatControl
{
    ...
    CGFloat marginY = (KMainH - scanW - 64-49 - padding - labelH) * 0.5;
    ...
    //选项栏
    UIView *tabBarView = [[UIView alloc] initWithFrame:CGRectMake(0, KMainH - tabBarH - 64 - 49, KMainW, tabBarH)];
...
//        NSLog(@"扫描结果:%@",[[metadataObjects firstObject] stringValue]);
        resScanStr = [[metadataObjects firstObject] stringValue];
        [[RvcNetWork shared] checkScanCodeWithLoginID:[UIData sharedClient].loginID withBarcode:[[metadataObjects firstObject] stringValue] withUserType:1 withSessionSid:[[RvcNetWork shared] nextSessionID]];
    }
}
...
- (void)netSocketClient:(id)netSocket withMethod:(int)method didReceivedData:(id)data{
    dispatch_async(dispatch_get_main_queue(), ^{
        [self parseCheckScanBarcodeBind:data];
        [self parseScanServiceList:data];
        [self parseCheckOrderBarcodeBind:data];
    });
}

- (void)parseScanServiceList:(TcpResponse*)response{
...
- (void)parseCheckScanBarcodeBind:(TcpResponse*)response{
    if (kCheckScanCodeResponse != response.tag)
    {
        return;
    }
    id data = response.responseData;
    ...
    CheckScanCodeInfo * infoScanCode = data;
    if (infoScanCode.orderId == 0) {
        ...
        _serviceInfo = [_serviceArr objectAtIndex:0];
        comOrderVC.comInfo = _serviceInfo;
        [self.navigationController pushViewController:comOrderVC animated:YES];
    }else if (infoScanCode.orderId > 0){
        if (infoScanCode.status == 1) {
            [[UIWaitView sharedClient] showBlockCanSelectPrompt:@"您已购买了服务，请存钥匙" withImage:@"success" withItem:@"是" withItem2:@"否" complement:^(NSInteger sel) {
                if (sel == 0) {
                    [[RvcNetWork shared] orderBarcodeBindWithLoginID:[UIData sharedClient].loginID withOrderId:infoScanCode.orderId withBarcode:resScanStr withBindType:1 withSessionSid:[[RvcNetWork shared] nextSessionID]];
                }else{
                    self.tabbarBlo(0);
                }
            }];
        }else if (infoScanCode.status == 6){
            [[RvcNetWork shared] orderBarcodeBindWithLoginID:[UIData sharedClient].loginID withOrderId:infoScanCode.orderId withBarcode:resScanStr withBindType:1 withSessionSid:[[RvcNetWork shared] nextSessionID]];
        }
    }
}

- (void)parseCheckOrderBarcodeBind:(TcpResponse*)response{
